...
# T00 cross-mapping M20 (i.e. testing M20 as a cause of T00)                  ##¡ï¡ïChange the variables here
# Determine the embedding dimension
E.test.x=NULL
for(E.t in 2:8){
  cmxy.t <- ccm(dac.n, E = E.t, lib_column = "T00", target_column = "M20",    ##¡ï¡ïChange the variables here
...
x_xmap_y <- ccm(dac.n, E=E_x,lib_column="T00", target_column="M20",           ##¡ï¡ïChange the variables here
...
########################################################### B¡úA
# M20 cross-mapping T00 (i.e. testing T00 as a cause of M20)                  ##¡ï¡ïChange the variables here
# Determine the embedding dimension
E.test.y=NULL
for(E.t in 2:8){
  cmxy.t <- ccm(dac.n, E = E.t, lib_column = "M20", target_column = "T00",    ##¡ï¡ïChange the variables here
                lib_sizes = 752, num_samples = 1,tp=-1,random_libs = F)       ##¡ø¡øChange the atotal mount here
  E.test.y=rbind(E.test.y,cmxy.t)}
(E_y <- E.test.y$E[which.max(E.test.y$rho)[1]])

# CCM analysis
y_xmap_x <- ccm(dac.n, E=E_y,lib_column="M20", target_column="T00",           ##¡ï¡ïChange the variables here
                lib_sizes=libs, num_samples=200, replace=T, RNGseed=2301)
...
legend(500,0.4,c("T00 xmap M20","M20 xmap T00"),lty=c(1,1),col=c("blue","red"))  ##¡ï¡ïChange the variables here
abline(h=cor(dac[,'T00'],dac[,'M20']),lty=3)                                     ##¡ï¡ïChange the variables here
print(cor(dac[,'T00'],dac[,'M20']))                                              ##¡ï¡ïChange the variables here