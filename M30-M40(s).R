...
# M30 cross-mapping M40 (i.e. testing M40 as a cause of M30)                  ##¡ï¡ïChange the variables here
# Determine the embedding dimension
E.test.x=NULL
for(E.t in 2:8){
  cmxy.t <- ccm(dac.n, E = E.t, lib_column = "M30", target_column = "M40",    ##¡ï¡ïChange the variables here
...
x_xmap_y <- ccm(dac.n, E=E_x,lib_column="M30", target_column="M40",           ##¡ï¡ïChange the variables here
...
########################################################### B¡úA
# M40 cross-mapping M30 (i.e. testing M30 as a cause of M40)                  ##¡ï¡ïChange the variables here
# Determine the embedding dimension
E.test.y=NULL
for(E.t in 2:8){
  cmxy.t <- ccm(dac.n, E = E.t, lib_column = "M40", target_column = "M30",    ##¡ï¡ïChange the variables here
                lib_sizes = 752, num_samples = 1,tp=-1,random_libs = F)       ##¡ø¡øChange the atotal mount here
  E.test.y=rbind(E.test.y,cmxy.t)}
(E_y <- E.test.y$E[which.max(E.test.y$rho)[1]])

# CCM analysis
y_xmap_x <- ccm(dac.n, E=E_y,lib_column="M40", target_column="M30",           ##¡ï¡ïChange the variables here
                lib_sizes=libs, num_samples=200, replace=T, RNGseed=2301)
...
legend(500,0.4,c("M30 xmap M40","M40 xmap M30"),lty=c(1,1),col=c("red","blue"))  ##¡ï¡ïChange the variables here

print(cor(dac[,'M30'],dac[,'M40']))                                              ##¡ï¡ïChange the variables here